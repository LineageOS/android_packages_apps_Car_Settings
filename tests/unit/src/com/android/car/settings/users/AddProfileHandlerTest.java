...
import static com.android.car.settings.users.AddProfileHandler.CONFIRM_CREATE_NEW_USER_DIALOG_TAG;
...
public class AddProfileHandlerTest {
    ...
    @Test
    public void userRestricted_canNotAddUser() {
        when(mUserManager.hasUserRestriction(UserManager.DISALLOW_ADD_USER)).thenReturn(true);

        assertThat(mAddProfileHandler.canAddUser(mUserManager)).isFalse();
    }

    @Test
    public void userNotRestricted_canAddUser() {
        when(mUserManager.hasUserRestriction(UserManager.DISALLOW_ADD_USER)).thenReturn(false);

        assertThat(mAddProfileHandler.canAddUser(mUserManager)).isTrue();
    }
    ...
    public void newUserConfirmed_invokesCreateNewUser()
        ...
        mAddProfileHandler.mConfirmCreateNewUserListener.onConfirm(/* arguments= */ null);
        // wait for async task
        mAddProfileHandler.mAddNewUserTask.get(ADD_USER_TASK_TIMEOUT, TimeUnit.SECONDS);
