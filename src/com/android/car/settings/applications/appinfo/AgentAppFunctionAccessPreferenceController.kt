...
/**
 * Manages a preference that links to the app function access screen for a specific agent app.
 *
 * The preference's visibility is conditional. It will only be shown if the necessary feature
 * flags are enabled and the target application is recognized as a valid agent by the
 * [AppFunctionManager].
 *
 ...
 * @param appFunctionManager The system service used to validate agent applications.
 */
class AgentAppFunctionAccessPreferenceController @JvmOverloads constructor(
    context: Context,
    ...
    override val action: String = AppFunctionManager.ACTION_MANAGE_AGENT_APP_FUNCTION_ACCESS

    override fun isValidPackage(packageName: String): Boolean {
        return appFunctionManager?.validAgents?.contains(packageName) ?: false
    }
}
